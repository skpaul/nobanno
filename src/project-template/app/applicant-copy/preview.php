...
Required::Logger()
    ...
    ->AgeCalculator(2)
    ->Imaging()
    ->UniqueCodeGenerator()->ExclusivePermission()->HttpHeader()->headerBrand()->applicantHeaderNav()->footer();
...
if (!isset($_GET["session-id"]) || empty(trim($_GET["session-id"]))) {
    HttpHeader::redirect(BASE_URL . "/sorry.php?msg=Invalid session. Please login again.", false);
}
...
$sql = "SELECT * from `lc_enrolment_registrations` WHERE registrationId=$applicant->registrationId";
$registration = ($db->select($sql))[0];

$sql = "SELECT * FROM `lc_enrolment_higher_educations` WHERE cinfoId=:cinfoId";
$hEdu = ($db->select($sql, array("cinfoId" => $cinfoId)))[0];

$sql = "SELECT * FROM `lc_case_list` WHERE userId=:userId AND caseType='criminal'";
$criminalCaseList = $db->select($sql, array("userId" => $applicant->userId));

$sql = "SELECT * FROM `lc_case_list` WHERE userId=:userId AND caseType='civil'";
$civilCaseList = $db->select($sql, array("userId" => $applicant->userId));

$smsKeyword = "BAR";

$hours = $postConfig->feePaymentHours;
$lastPaymentDate = $clock->toDate($applicant->appliedDatetime);
// $lastPaymentDate->add(new DateInterval('P3D')); //Add 3days.

//Add hours from post_configurations table.
$lastPaymentDate->add(new DateInterval("PT{$hours}H")); // $dt->add(new DateInterval('PT1H'));
...
<head>
    ...
    <?php
    Required::gtag()->metaTags()->favicon()->css();
...
<body>
    <div id="master-wrapper">
        ...
        <main class="previewPage">
            ...
            <div style="max-width: 1320px; margin-bottom:30px;">
                ...
                <div class="text-center margin-bottom-25">
                    ...
                    ?>
                        <section class="congratulation">
                            <h1 style="color:#32ba32;">!! Congratulations !!</h1>
                            <p style="font-weight: 600; margin-bottom: 36px;">Application submitted successfully.</p>
                        </section>
                    <?php
                    }
                    ?>
                    ...
                    ?>
                    <!-- <p class="delete-note">
                            This application is non-editable. If you find any information incorrect here, you can delete this application and resubmit the form. An application can not be deleted after the fee payment.</br>
                        </p>   -->
                    <?php
                    //  }
                    ?>
            ...
            <div style="width: 660px; margin:0 auto; margin-bottom:20px; display:flex; justify-content:space-between;">
                ...
                // if($applicant->fee==0)   {
                ?>
                <!-- <a class="btn btn-danger btn-large" style="text-decoration: none;" href=".php?session-id=<?= $encSessionId ?>&config-id=<?= $encConfigId ?>">Delete Application</a> -->
                <?php
                //  }
                ?>
            </div>
            <div id="pdfdiv" class="non-responsive printable applicant-copy-preview" style="border: 1px solid; margin:0 auto; width: 660px; padding: 10px 20px;">
                ...
                <?php
                    $formHeader = "";
                    if (strtolower($applicant->applicantType) == "re-appeared") {
                        $formHeader = "<h1>Re-Appear Application Form for Enrolment Examination</h1> <br><br>";
                    } else {
                ...
                <section class="formHeader" style="position: relative;">
                    <?php
                        echo $formHeader;
                    ?>
                    ...
                    <?php
                        if ($applicant->fee == 1) {
                    ?>
                        <img src="paid-icon.png" alt="" srcset="" style="   position: absolute; left: auto; top: 0; height: 72px;">
                    <?php
                        }
                    ?>
                </section>
                ...
                <section>
                    ...
                    <div class="grid">
                        ...
                        <!-- <div class="col-auto">
                            <div class="field border-left">
                                <label class="border-right">Pupilage Duration</label>
                                <div class="text"></div>
                            </div>
                        </div> -->
                    </div>
                            ...
                            <table class="table">
                                ...
                                <tbody>
                                    <tr>
                                        ...
                                        <?php
                                        if (strtolower($applicant->sscExamName) == "o level") {
                                            $sscResultContent = $applicant->oLevelResultDetails;
                                        } else {
                                    ...
                                    <tr>
                                        ...
                                        <?php
                                        if (strtolower($applicant->hscExamName) == "a level") {
                                            $hscResultContent = $applicant->aLevelResultDetails;
                                        } else {
                                ...
                                <table class="table">
                                    <thead>
                                        <tr style="border-bottom: 1px solid gray;">
                                            <th colspan="6">List of 5 Criminal Cases</th>
                                        </tr>
                                        <tr>
                                            <th>SL</th>
                                            <th>Case Number with Section</th>
                                    ...
                                    <tbody>
                                        <?php
                                            $criminalIndex = 1;
                                            foreach ($criminalCaseList as $case) {
                                        ?>
                                        <tr>
                                            <td><?=$criminalIndex++?></td>
                                            <td><?=$case->caseNumberWithSection; ?></td>
                                            <td><?=$case->nameOfTheCourt; ?></td>
                                            <td><?=$case->nameOfTheParties; ?></td>
                                            <td><?=$case->onBehalfOf; ?></td>
                                            <td><?=$case->presentPosition; ?></td>
                                        </tr>
                                        <?php
                                            }
                                        ?>
                        ...
                        <div class="grid">
                            <div class="col-auto">
                                <table class="table">
                                    <thead>
                                        <tr style="border-bottom: 1px solid gray;">
                                            <th colspan="6">List of 5 Civil Cases</th>
                                        </tr>
                                        <tr>
                                            <th>SL</th>
                                            <th>Case Number with Section</th>
                ...
                ?>
                    <div class="smsContent">
                        <hr style="margin-bottom: 5px; border: 1px dashed #bfbaba;">
                        <p>
                            <strong><?= $applicant->userId ?></strong> is your User ID, Please keep this number to pay the application fee Tk. <?= $applicant->requiredFee ?> within <strong><?= $clock->toString($lastPaymentDate, DatetimeFormat::Custom("d-m-Y h:i A"))  ?></strong> from any Teletalk prepaid mobile phone by SMS.
                        </p>
                        <p>
                            1st SMS Format: <?= $smsKeyword ?><?= htmlspecialchars("<space>UserID") ?> and send SMS to 16222
                            [Example : <?= $smsKeyword ?> <?= $applicant->userId ?>]. 1st-SMS Reply: Returns a PIN (8 Digits) like 23423495
                        </p>
                        <p>
                            2nd SMS Format: <?= $smsKeyword ?><?= htmlspecialchars("<space>YES<space>PIN") ?> and send SMS to 16222
                            [Example : <?= $smsKeyword ?> YES 23423495]. 2nd SMS Reply: Returns a USER ID & PASSWORD
                        </p>
                    </div>
                <?php
                }
            ...
            ?>
                <div class="wrapper">
                    <section class="paymentInstruction">
                        <h2>Payment Instruction</h2>
                        <p>
                        <ul>
                            <li>Application fee is <span>Tk. <?= $applicant->requiredFee ?></span></li>
                            <li>Application fee payment deadline <span><?= $clock->toString($lastPaymentDate, DatetimeFormat::Custom("d-m-Y h:i A")) ?></span>.</li>
                            <li>SMS must be sent from any <span>Teletalk prepaid number</span>.</li>
                            <li>SMS must be in valid format. Please note the following instructions - </li>
                        </ul>
                        </p>
                        <article class="firstSms">
                            <div class="smsInstruction">
                                <div class="right">
                                    <div class="top">
                                        <div class="title">
                                            <div class="text">1st SMS</div>
                                            <div class="img">
                                                <img src="sms.png" alt="">
                                            </div>
                                        </div>
                                        <div class="step">
                                            <p>write a new sms</p>
                                            <p><?= $smsKeyword . htmlspecialchars("<space>UserId") ?></p>
                                        </div>
                                        <div class="arrow">
                                            <div>
                                        ...
                                        <div class="step">
                                            <p>
                                                You will get a reply SMS with a PIN
                                            </p>
                                        </div>
                                    </div>
                                    <div class="bottom">
                                        <div class="title">1st SMS </br> example</div>
                                        <div class="step">
                                            <div class="mobileScreen">
                                                <div class="outer">
                                                    <div class="inner">
                                                        <?= $smsKeyword . " " . $applicant->userId ?>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                        ...
                        <article class="2ndSms">
                            <div class="smsInstruction">
                                <div class="right">
                                    <div class="top">
                                        <div class="title">
                                            <div class="text">2nd SMS</div>
                                            <div class="img">
                                                <img src="sms.png" alt="">
                                            </div>
                                        </div>
                                        <div class="step">
                                            <p>write a new sms- </p>
                                            <!-- <p>NGI30spaceYESspacePIN</p> -->
                                            <p><?= $smsKeyword . htmlspecialchars("<space>YES<space>PIN") ?></p>
                                        </div>
                                        <div class="arrow">
                                            <div>
                                                <img src="arrow.png" alt="">
                                        ...
                                        <div class="step">
                                            <p>
                                                You will get a reply SMS with new User ID and Password
                                            </p>
                                        </div>
                                    </div>
                                    <div class="bottom">
                                        <div class="title">2nd SMS </br> example</div>
                                        <div class="step">
                                            <div class="mobileScreen">
                                                <div class="outer">
                                                    <div class="inner">
                                                        <?= $smsKeyword ?> YES 11223456
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
