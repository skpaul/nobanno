<?php
    ...
    class Database 
    {
        ...
        #region INSERT
             ...
             * Insert operation.
             * 
             * @param string $tableNameOrSqlStatement  An SQL statement or table name
             * @param array $data  i.e. $data["colName1"] = intValue, $data["colName2"] = "stringValue". In case of Class, use get_object_vars($class)
             * 
             * @return integer last Insert Id
             */

            public function insert(string $tableNameOrSqlStatement, array $data = array()):int{
                //str_starts_with() only available in php8 or newer
                if (str_starts_with($tableNameOrSqlStatement, "insert into") || str_starts_with($tableNameOrSqlStatement, "INSERT INTO")) {
                    //$tableNameOrSqlStatement is now an SQL statement
                    try {
                        $result = $this->_insert($tableNameOrSqlStatement, $data);
                    } catch (\Throwable $th) {
                    ...
                    //$tableNameOrSqlStatement is now a tableName.
                    try {
                        $sql = $this->_prepareInsertSqlStatment($data, $tableNameOrSqlStatement);
